...
两个服务共用环境变量校验、Cron 调度、日志、HTML 邮件、重试延时和时区格式化逻辑。
...
- `env/common.env`：两个服务共用的时区和 SMTP 配置。
...
| 变量 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `TZ` | 否 | `Asia/Shanghai` | Cron 和邮件时间使用的 IANA 时区 |
| `SMTP_HOST` | 是 | - | SMTP 主机 |
...
任务重叠时，新触发会被跳过并发送 `Skipped` 邮件，不会排队。检测到 Cron 漏执行时会立即补跑；如果此时仍有任务运行，补跑也会按重叠规则跳过。
